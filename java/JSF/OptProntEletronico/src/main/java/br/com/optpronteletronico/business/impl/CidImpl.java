 ...
 * @author evandro
 */
public class CidImpl implements ICid {
    ...
    public Cid recuperarCidPorId(Long id) throws OPTException {
        try {
            Root<Cid> from = this.optImpl.from(Cid.class);
            ...
            Cid object = (Cid) qry.getSingleResult();
            return object;
        } catch (Exception e) {
    ...
    public List<Cid> recuperarTodosRegistrosPaginacao(Cid cid, Integer pag, Integer maxReg) throws OPTException {
        ...
        Query qry = this.em.createQuery(" SELECT c FROM Cid c ");

        Query qryMax = this.em.createQuery(" SELECT Count(c) FROM Cid c ");

        qry.setFirstResult((pag - 1) * maxReg);
        qry.setMaxResults(maxReg);

        this.total = Integer.parseInt(qryMax.getSingleResult().toString());
        List<Cid> result = qry.getResultList();
        return result;
    }
