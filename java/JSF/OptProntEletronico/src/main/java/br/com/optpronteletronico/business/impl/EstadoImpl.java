...
public class EstadoImpl implements IEstado {
    ...
    public List<Estado> recuperarTodosRegistrosPaginacao(Estado estado, Integer pag, Integer maxReg) throws OPTException {
        ...
        Query qry = this.em.createQuery(" SELECT c FROM Estado c "
                + " LEFT JOIN FETCH c.pais");

        Query qryMax = this.em.createQuery(" SELECT Count(c) FROM Estado c ");

        qry.setFirstResult((pag - 1) * maxReg);
        qry.setMaxResults(maxReg);

        this.total = Integer.parseInt(qryMax.getSingleResult().toString());
        List<Estado> result = qry.getResultList();
        return result;
    }
    ...
    public Estado recuperarEstadoPorId(Long id) throws OPTException {
        try {
            Root<Estado> from = this.optImpl.from(Estado.class);
            from.fetch("pais", JoinType.LEFT);
            ...
            Estado object = (Estado) qry.getSingleResult();
            return object;
        } catch (Exception e) {
