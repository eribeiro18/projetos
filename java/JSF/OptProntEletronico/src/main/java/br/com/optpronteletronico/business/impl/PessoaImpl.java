...
public class PessoaImpl implements IPessoa {
    ...
    @Override
    public List<Pessoa> recuperarTodosRegistrosPaginacao(Pessoa pessoa, Integer pag, Integer maxReg) throws OPTException {

        Query qry = this.em.createQuery(" SELECT c FROM Pessoa c "
                + " LEFT JOIN FETCH c.tipo ");

        Query qryMax = this.em.createQuery(" SELECT Count(c) FROM Pessoa c ");

        qry.setFirstResult((pag - 1) * maxReg);
        qry.setMaxResults(maxReg);

        this.total = Integer.parseInt(qryMax.getSingleResult().toString());
        List<Pessoa> result = qry.getResultList();
        return result;
    }
    ...
    public Pessoa recuperarPessoaPorId(Long id) throws OPTException {
        try {
            Root<Pessoa> from = this.optImpl.from(Pessoa.class);
            from.fetch("tipo", JoinType.LEFT);
            ...
            Pessoa object = (Pessoa) qry.getSingleResult();
            return object;
        } catch (Exception e) {
