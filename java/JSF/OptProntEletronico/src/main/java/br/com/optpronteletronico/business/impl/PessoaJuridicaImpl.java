 ...
 * @author evandro
 */
public class PessoaJuridicaImpl implements IPessoaJuridica {

    private @Inject
    EntityManager em;
    private @Inject
    IOpt<PessoaJuridica> optImpl;
    ...
    public List<PessoaJuridica> recuperarTodosRegistrosPaginacao(PessoaJuridica pessoaJuridica, Integer pag, Integer maxReg) throws OPTException {
        ...
        Query qry = this.em.createQuery(" SELECT c FROM PessoaJuridica c "
                + " LEFT JOIN FETCH c.pessoa ");

        Query qryMax = this.em.createQuery(" SELECT Count(c) FROM PessoaJuridica c ");
    ...
    public PessoaJuridica recuperarPessoaJuridicaPorId(Long id) throws OPTException {
        try {
            Root<PessoaJuridica> from = this.optImpl.from(PessoaJuridica.class);
            ...
            PessoaJuridica object = (PessoaJuridica) qry.getSingleResult();
            return object;
        } catch (Exception e) {
