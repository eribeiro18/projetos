 ...
 * @author evandro
 */
public class ProntuarioImpl implements IProntuario {

    private @Inject
    EntityManager em;
    private @Inject
    IOpt<Prontuario> optImpl;
    ...
    public List<Prontuario> recuperarTodosRegistrosPaginacao(Prontuario prontuario, Integer pag, Integer maxReg) throws OPTException {
        ...
        Query qry = this.em.createQuery(" SELECT c FROM Prontuario c "
                + " LEFT JOIN FETCH c.consulta ");

        Query qryMax = this.em.createQuery(" SELECT Count(c) FROM Prontuario c ");

        qry.setFirstResult((pag - 1) * maxReg);
        qry.setMaxResults(maxReg);

        this.total = Integer.parseInt(qryMax.getSingleResult().toString());
        List<Prontuario> result = qry.getResultList();
        return result;
    }
    ...
    public Prontuario recuperarProntuarioPorId(Long id) throws OPTException {
        try {
            Root<Prontuario> from = this.optImpl.from(Prontuario.class);
            ...
            Prontuario object = (Prontuario) qry.getSingleResult();
            return object;
        } catch (Exception e) {
