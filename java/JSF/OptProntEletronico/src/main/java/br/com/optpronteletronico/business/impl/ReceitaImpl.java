...
public class ReceitaImpl implements IReceita {
    ...
    public List<Receita> recuperarTodosRegistrosPaginacao(Receita posConsulta, Integer pag, Integer maxReg) throws OPTException {
        ...
        Query qry = this.em.createQuery(" SELECT c FROM Receita c "
                + " LEFT JOIN FETCH c.consulta ");

        Query qryMax = this.em.createQuery(" SELECT Count(c) FROM Receita c ");

        qry.setFirstResult((pag - 1) * maxReg);
        qry.setMaxResults(maxReg);

        this.total = Integer.parseInt(qryMax.getSingleResult().toString());
        List<Receita> result = qry.getResultList();
        return result;
    }
    ...
    public Receita recuperarReceitaPorId(Long id) throws OPTException {
        try {
            Root<Receita> from = this.optImpl.from(Receita.class);
            ...
            Receita object = (Receita) qry.getSingleResult();
            return object;
        } catch (Exception e) {
